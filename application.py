...
def editCompany(company_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("edit Company successfully.!")
        return redirect(url_for('menuCompany'))
    else:
...
def deleteCompany(company_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("delete Company successfully.!")
...
def menuCars(company_id):
    company = session.query(Company).filter_by(id=company_id).one()
    carItems = session.query(MenuCars).filter_by(company_id=company.id)
    return render_template('menuCars.html', company=company, carItems=carItems)
...
def addNewCar(company_id):
    ...
    if request.method == 'POST':
        newCars = MenuCars(
         ...
         price=request.form['price'], company_id=company_id)
        session.add(newCars)
        session.commit()
...
def editInforamtionCar(company_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("edit information the Car successfully.!")
        return redirect(url_for('menuCars', company_id=company_id))
    else:
...
def deleteInforamtionCar(company_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("delete the Car successfully.!")
        return redirect(url_for('menuCars', company_id=company_id))
    else:
